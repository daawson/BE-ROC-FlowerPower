        ...
        <div id="main-container" class="block-full">
            <div class="block-50">                
                <form id="loginForm" class="register" action="php/register_check.php" method="POST">
                    ...
                    ?>
                    Email<input min-length="6" max-length="64" placeholder="email@email.nl" pattern="^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$" type="email" name="email" id="email" required><br>
                    ...
                    Adress<input min-length="4" max-length="128" placeholder="Straatnaam en huisnummer" pattern="^([1-9][e][\s])*([a-zA-Z]+(([\.][\s])|([\s]))?)+[1-9][0-9]*(([-][1-9][0-9]*)|([\s]?[a-zA-Z]+))?$" max-length="64" type="text" name="adress" id="adress" required><br>                    
                    Postcode<input max-length="7" placeholder="1234 AB" pattern="^([0-9]{4}[ ]+[a-zA-Z]{2})$" type="text" name="postcode" id="postcode" required><br>
                    
                    Telefoonnummer<input min-length="8" placeholder="0612345678" pattern="(^\+[0-9]{2}|^\+[0-9]{2}\(0\)|^\(\+[0-9]{2}\)\(0\)|^00[0-9]{2}|^0)([0-9]{9}$|[0-9\-\s]{10}$)" type="text" name="phone" id="phone" required><br>
